"""
Flask routes for the face recognition application
"""
import os
import logging
import cv2
...
def register_routes(app, face_service):
    ...
    @app.route('/delete_all_persons', methods=['DELETE'])
    def delete_all_persons():
        """Delete all persons from the recognition database"""
        try:
            success = face_service.db_service.delete_all_persons()
            ...
            if success:
                ...
                return jsonify({
                    "status": "error",
                    "message": "Failed to delete all persons"
                }), 500
        except Exception as e:
